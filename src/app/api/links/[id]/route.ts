...
import { deleteLink, getLinkById, updateLink } from '@/actions/links'
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!result.success) {
      return NextResponse.json({ success: false, error: result.error || 'Error fetching link' }, { status: 404 })
    }

    return NextResponse.json({ success: true, link: result.link }, { status: 200 })
  } catch (error) {
